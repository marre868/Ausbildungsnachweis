...
DATA_FILE = writable_path("data.json")
DEFAULT_TEMPLATE = (
    writable_path("Vorlage.docx")
    if os.path.exists(writable_path("Vorlage.docx"))
    else resource_path("Vorlage.docx")
)
...
def open_browser(host: str, port: int) -> None:
    """Open the local Flask page in the user's default browser."""
...
if __name__ == "__main__":
    ...
    debug = os.environ.get("FLASK_DEBUG", "1") == "1" and not running_as_exe()

    # Beim Debug-Reload startet Werkzeug den Prozess zweimal. Die Seite soll nur
    # im eigentlichen Server-Prozess automatisch geöffnet werden.
    if os.environ.get("WERKZEUG_RUN_MAIN") == "true" or not debug:
        open_browser(host, port)

    print(f"Öffne die Weboberfläche automatisch: http://{host}:{port}")
